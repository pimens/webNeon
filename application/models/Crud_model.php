	...
	*/
	...
	function create_video()
	{
		...
		if (empty($_FILES["file_video"]["name"]) || empty($_FILES["thumbnail"]["name"]) ) {
			...
			//thum upload
			 $con1 = array();
			 $con1['upload_path']          = './assets/global/video_thumb';
			 $con1['allowed_types']        = 'gif|jpg|png';
			 $con1['max_size']             = '1000M';
			 $con1['remove_space']         = TRUE;
			 $con1['file_name']			= $_FILES["thumbnail"]["name"];
			 $this->load->library('upload', $con1, 'thumb'); // Create custom object for cover upload
			//  $this->thumb->initialize($con1);
			 $this->thumb->do_upload('thumbnail');
			 $file2      = $this->thumb->data();
			 $file2      = $file2['file_name'];
